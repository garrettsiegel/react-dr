...
import { filterSourceFiles, getDiffInfo, isSafeGitRefName } from "./utils/get-diff-files.js";
...
const program = new Command()
  ...
  .action(async (directory: string, flags: CliFlags) => {
    ...
    try {
      ...
      if (!isScoreOnly) {
        logger.log(`react-dr-cli v${VERSION}`);
        logger.break();
      }
      ...
      if (explicitBaseBranch && !isSafeGitRefName(explicitBaseBranch)) {
        throw new Error(`Invalid diff base branch: ${explicitBaseBranch}`);
      }
...
${highlighter.dim("Learn more:")}
  ${highlighter.info("https://github.com/garrett/react-dr")}
`,
  );
